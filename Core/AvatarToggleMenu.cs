    ...
    public class AvatarToggleMenu : AvatarMenuBase
    {
        ...
        protected override void OnMainGUI(IList<string> children)
        {
            ...
            foreach (var child in children)
            {
                ...
                var existNames = ToggleBlendShapes.Names(child).ToList();
                var names = gameObjectRef == null ? existNames : Util.GetBlendShapeNames(gameObjectRef).UnionBlendShapeNames(existNames);
                var existParameters = ToggleShaderParameters.Names(child).ToFakeShaderFloatParameters().Concat(ToggleShaderVectorParameters.Names(child).ToFakeShaderVectorParameters()).ToList();
                    ...
                    if (EditorGUILayout.ToggleLeft(member.Description, true))
                    {
                        ...
                        using (new EditorGUILayout.HorizontalScope())
                        {
                            if (member.MemberType == typeof(float))
                            {
                                ...
                                ValuePickerButton(child, member, ref newValue.Inactive);
                                ValueApplyButton(child, member, newValue.Inactive);
                                newValue.Active = EditorGUILayout.FloatField("ON", (float)value.Active, GUILayout.Width(110));
                                ValuePickerButton(child, member, ref newValue.Active);
                                ValueApplyButton(child, member, newValue.Active);
                            }
